	...
	scheduler._render_x_header = function(i, left, d, h) {
		...
		if(!pr || pr.days <= 1) {
			...
			if( (i+1)%units_l === 0 ) {
				...
				head.innerHTML = this.templates[this._mode + "_second_scale_date"](date, this._mode); //TODO - move in separate method
				h.appendChild(head);
			}
	...
	scheduler.locate_holder_day=function(a,b,ev){
		...
		if (pr && ev) {
			...
			if(pr.days <= 1){
				...
				var dx = 24*60*60*1000;
				var ind = Math.floor((ev.start_date.valueOf()-1 - scheduler._min_date.valueOf())/dx); // -1 for events with 00:00 end time
				var units_l = pr.options.length;
				var section_index = pr.order[ev[pr.map_to]];
